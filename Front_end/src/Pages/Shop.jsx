...
const BookShop = () => {
  ...
  // Filter books based on selected category, search query, and price range
  const filterBooks = books.filter((book) => {
    const matchesCategory = selectedCategory === "All" || book.category === selectedCategory;
    const matchesSearch =
  ...
  // Navigate to BookDetails page when "Book Now" button is clicked
  const handleBookNow = (book) => {
    navigate(
      `/bookdetails?bookTitle=${encodeURIComponent(book.title)}&bookAuthor=${encodeURIComponent(book.author)}&bookImage=${encodeURIComponent(book.imageUrl)}&bookPrice=${encodeURIComponent(book.price)}&bookDescription=${encodeURIComponent(book.description)}`
    );
  };
    ...
    >
        ...
        <div className="mb-8">
          ...
          <div className="mt-4">
            ...
            <div className="flex justify-between text-sm text-gray-500">
              <span>{priceRange[0]} RS</span>
              <span>{priceRange[1]} RS</span>
            </div>
          </div>
        </div>
      )}
      ...
      >
        {filterBooks.length > 0 ? (
            ...
            >
              ...
              <div className="absolute left-0 right-0 bottom-16 flex justify-center">
                <div className="inline-block bg-white w-3/4 px-4 py-2 shadow-md rounded-lg">
                  ...
                  <p className="mt-2 text-lg font-bold text-gray-900">RS: {book.price}</p>
                </div>
              </div>
              ...
              <motion.button
                ...
                onClick={() => handleBookNow(book)} // Navigate to Book Details page
              >
                Book Now
              </motion.button>
            </motion.div>
          ...
          <p className="text-center text-xl text-gray-600 col-span-full">No books available</p>
        )}
      </motion.div>
    </motion.div>
  );
